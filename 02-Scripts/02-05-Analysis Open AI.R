...
# Step 3: Initialize response vector
# Initialisieren der Artikel und Antwortvektoren
chatgpt_responses <- vector("character", length = nrow(articles))

# Definiere eine Funktion mit exponentiellem Backoff für Rate Limiting
exponential_backoff <- function(retries) {
  # Wartet 2^retries Sekunden (Exponentielles Backoff)
  Sys.sleep(2^retries)
}
...
for (i in seq_len(nrow(articles))) {
  ...
  while (!success && retries < 5) {  # Maximal 5 Versuche
    response <- tryCatch({
      httr::POST(
    ...
    if (!is.null(response)) {
      ...
      if (!is.null(response_content$choices) && length(response_content$choices) > 0) {
        ...
        success <- TRUE  # Um nicht unendlich zu wiederholen, falls keine Wahl vorhanden ist
      }
    }
    ...
    if (!success) {
      ...
      exponential_backoff(retries)  # Warten nach jedem fehlgeschlagenen Versuch
    }
  }
